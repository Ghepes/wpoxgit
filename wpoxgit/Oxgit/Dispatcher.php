...
use Oxgit\Storage\PackageModel;
...
class Dispatcher
{
    ...
    {
        if (isset($_POST['wpoxgit'])) {

            if ( ! current_user_can('update_plugins') || ! current_user_can('update_themes') ) {
                wp_die(__('You do not have sufficient permissions to access this page.'));
            }

            $request = $_POST['wpoxgit'];
    ...
    {
        if ( ! isset($_GET['wpoxgit-hook']))
            return;

        if ( ! isset($_GET['token']) || $_GET['token'] !== get_option('wpoxgit_token')) {
