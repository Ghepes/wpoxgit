...
<p>If you're having difficulty getting your copy of WP Oxgit working, you're in the right place. Here you can check on how all the various pieces of the plugin are working. In addition to this screen, you can enable logging in the <a href="<?php echo admin_url('admin.php?page=wpoxgit&tab=log'); ?>">"Logging" tab</a> and see if there are any unexpected messages in the logs there.</p>

<p>If one of the below tests fails, copy/paste the below output and send it to <a href="mailto:hi@wpoxgit.com">hi@wpoxgit.com</a> and we'll be able to start debugging your problem more quickly.</p>

<h2>Active License Key</h2>
<p>This test determines whether you have registered a license key with WP Oxgit and it is currently activated in your installation.</p>

<?php $key = get_option('wpoxgit_license_key', false); ?>

<?php if( $key ) : ?>
    <strong style="color: green;">Status: Passed.</strong> You're all good!
<?php else : ?>
    <strong style="color: red;">Status: Failed.</strong> It doesn't look like you have a license key registered with this installation of WordPress. Do you need to <a href="https://dashboard.wpoxgit.com/checkout?plan=price_1JO1NpEJPTdj0nOdAMsphyOy" target="_blank">buy one</a> or <a href="https://dashboard.wpoxgit.com/dashboard" target="_blank">look up your existing license key</a>?
<?php endif; ?>

<h2>cURL Information</h2>
<p>This test determines what version of cURL your server is running. To check the age of your current cURL version as well as to check what the latest version is, <a href="https://curl.se/docs/releases.html" target="_blank">take a look here.</a> If you are running an older version of cURL, you may have issues connecting to the WP Oxgit server and getting your license key activated. If this is the case, you may want to ask your web host if they can upgrade the version of cURL that your server is running.</p>
...
<?php
    $baseUrl = 'https://dashboard.wpoxgit.com/ping/';
...
<?php 
    ...
    if( ! $token ) : ?>
        <strong style="color: red;">Status: Failed.</strong> It looks like you don't have a GitHub token stored. If you are trying to use GitHub, <a href="<?php echo admin_url('admin.php?page=wpoxgit&tab=github'); ?>">generate a GitHub token here</a>.
    <?php elseif( is_object( $response ) && 'Bad credentials' === $response->message ) : ?>
        <strong style="color: red;">Status: Failed.</strong> It looks like you GitHub token isn't valid. Try and <a href="<?php echo admin_url('admin.php?page=wpoxgit&tab=github'); ?>">regenerate one here</a> and make sure you have pasted it into the GitHub token field.
    <?php else : ?>
        <strong style="color: green;">Status: Passed.</strong> You're all good!
    <?php endif; ?>
...
<?php  
    ...
    $response = wp_remote_get("https://cloud.wpoxgit.com/auth/bitbucket/refresh-token?refresh_token={$token}");
    ...
    if( ! $token ) : ?>
        <strong style="color: red;">Status: Failed.</strong> It looks like you don't have a Bitbucket token stored. If you are trying to use Bitbucket, <a href="<?php echo admin_url('admin.php?page=wpoxgit&tab=bitbucket'); ?>">generate a Bitbucket token here</a>.
    <?php elseif( 200 !== $code ) : ?>
...
<?php
    ...
    if( ! $baseUrl ) : ?>
        <strong style="color: red;">Status: Failed.</strong> No GitLab URL stored. If you're trying to use GitLab, <a href="<?php echo admin_url('admin.php?page=wpoxgit&tab=gitlab'); ?>">add your Gitlab information here.</a>
    <?php endif; ?>
    ...
    if ( ! $token ) : ?>
        <strong style="color: red;">Status: Failed.</strong> No GitLab token stored. If you're trying to use a private GitLab repository, <a href="<?php echo admin_url('admin.php?page=wpoxgit&tab=gitlab'); ?>">add your Gitlab information here.</a>
    <?php elseif( 200 !== $code ) : ?>
