...
<form method="post" action="<?php echo admin_url(); ?>options.php">
    <?php settings_fields('oxgit-enable-logging'); ?>
    <?php do_settings_sections('oxgit-enable-logging'); ?>
    <?php if (get_option('oxgit_logging_enabled') == 1) { ?>
        <input type="hidden" name="oxgit_logging_enabled" value="0">
        <?php submit_button('Disable logging'); ?>
    <?php } else { ?>
        <input type="hidden" name="oxgit_logging_enabled" value="1">
...
<?php if (get_option('oxgit_logging_enabled') == 1) { ?>
    ...
    <form method="post" action="" onsubmit="return confirm('The log is gonna be wiped clean. Sure about it?');">
        ...
        <input type="hidden" name="wpoxgit[action]" value="clear-log">
        <p class="submit">
