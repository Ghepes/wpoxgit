...
class ThemeRepository
{
    ...
    public function allOxgitThemes()
    {
        global $wpdb;
     ...
     */
    public function oxgitThemeFromStylesheet($stylesheet)
    {
        global $wpdb;

        $table_name = oxgitTableName();

        $model = new PackageModel(array('package' => $stylesheet));

        $row = $wpdb->get_row("SELECT * FROM $table_name WHERE type = 2 AND package = '{$model->package}'");
