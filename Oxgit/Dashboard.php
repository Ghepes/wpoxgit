...
use Oxgit\Commands\EditPlugin;
use Oxgit\Commands\EditTheme;
use Oxgit\Commands\InstallPlugin;
use Oxgit\Commands\InstallTheme;
use Oxgit\Commands\UnlinkPlugin;
use Oxgit\Commands\UnlinkTheme;
use Oxgit\Commands\UpdatePlugin;
use Oxgit\Commands\UpdateTheme;
use Oxgit\Commands\UpdatePackageFromWebhook;
use Oxgit\License\LicenseManager;
use Oxgit\Log\Logger;
use Oxgit\Storage\Database;
...
class Dashboard
{
     ...
     */
    public function __construct(Database $db, LicenseManager $license, Logger $log, PluginRepository $plugins, Oxgit $oxgit, ThemeRepository $themes)
    {
        $this->db = $db;
    ...
    public function getPlugins()
    {
        if (isset($_GET['package'])) {
            try {
                $plugin = $this->plugins->oxgitPluginFromFile($_GET['package']);
        ...
        $data['plugins'] = $this->plugins->allOxgitPlugins();
    ...
    public function getThemes()
    {
        if (isset($_GET['package'])) {
            try {
                $theme = $this->themes->oxgitThemeFromStylesheet($_GET['package']);
        ...
        $data['themes'] = $this->themes->allOxgitThemes();
    ...
    public function execute($command)
    {
        ...
        $handler = $this->oxgit->make($handlerClass);

        try {
            $handler->handle($command);
        } catch (Exception $e) {
            status_header(400);
            $this->messages[] = new WP_Error('wpoxgit_error', $e->getMessage());
    ...
    protected function render($view, $data = array())
    {
        ...
        $data['hasValidLicense'] = $this->oxgit->hasValidLicenseKey();
        $data['name'] = $this->oxgit->getName();
